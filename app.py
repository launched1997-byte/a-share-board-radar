...
def api_scan():
    try:
        ...
        return jsonify({'ok':False,'version':'5.0','source':'服务器异常','stats':{},'sector_rank':[],'top10':[],'candidates':[],'error':f'{type(e).__name__}: {e}'}),200
@app.get('/api/status')
def api_status():
    try:
...
@app.get('/health')
def health():return jsonify({'status':'ok','service':'a-share-board-radar','version':'5.0'})
