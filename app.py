...
def api_scan():
    try:
        ...
        return jsonify({'ok':False,'version':'4.0','source':'服务器异常','stats':{},'candidates':[],'error':f'{type(e).__name__}: {e}','note':'V4后端已捕获异常，没有使用模拟行情。'}),200
@app.get('/api/status')
def api_status():
    try:
...
@app.get('/health')
def health(): return jsonify({'status':'ok','service':'a-share-board-radar','version':'4.0'})
