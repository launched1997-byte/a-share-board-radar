...
def _eastmoney_snapshot():
    s = _session(); url = 'https://push2.eastmoney.com/api/qt/clist/get'
    params = {'pn':1,'pz':6000,'po':1,'np':1,'fltt':2,'invt':2,'fid':'f3','fs':'m:0+t:6,m:0+t:80,m:1+t:2,m:1+t:23','fields':'f12,f14,f2,f3,f4,f5,f6,f8,f10,f15,f16,f17,f18'}
...
def provider_status():
    ...
    for name,fn in [('iTick',_itick_snapshot),('新浪',_sina_snapshot),('东方财富',_eastmoney_snapshot)]:
        try:
            df,label=fn(); ok=df is not None and not df.empty
            result['providers'].append({'name':label,'ok':ok,'rows':int(len(df)) if df is not None else 0})
            result['ok'] |= ok
        except Exception as e: result['providers'].append({'name':name,'ok':False,'error':f'{type(e).__name__}: {e}'})
    return result


def get_limit_up_pool(): return pd.DataFrame()
def get_yesterday_limit_up(): return pd.DataFrame()
